...
def _is_below_ema8(ema8_status, ta_dict):
    """True if price is below EMA8 — entry trigger not fired, wait for reclaim."""
    if ema8_status and str(ema8_status).strip().lower() == "below":
        return True
    if ta_dict and isinstance(ta_dict, dict):
        pct = ta_dict.get("price_vs_ema8")
        if pct is not None:
            try:
                return float(pct) < 0
...
Context: 3 Stock Rotation (Trader One $20K cap): 60% Top 3 leveraged ETFs (weekly momentum pick) + 15% GDX + 15% FCX + 10% cash. Always hold GDX/FCX. 5% stop per rotation position. Universe: 26 leveraged ETFs (NVDU, TSLL, etc.) + S&P 500. Cash account.
...
- **CHART IMAGES** (when provided): Analyze price action, support/resistance, trend structure. Integrate chart patterns with the numerical data.
- **Leveraged ETFs (SOXL, TQQQ, etc.)**: MAX 3-DAY HOLD due to decay.
- **LEVERAGED TICKERS**: Suggest leveraged alternatives when applicable. For each recommended stock, add a line: "Leveraged alt: [TICKER] (3d max)" when one exists — e.g. NVDA→NVDU/NVDL, AAPL→AAPU, MSFT→MSFU, MU→MUU, sector→TQQQ/SOXL/ERX/FAS/CURE. Same thesis, amplified. ⚠️ 3-DAY MAX for leveraged.
...
#. [TICKER] - [COMPANY NAME] ⭐
   ...
   [Leveraged alt: TICKER (3d max) — when applicable]
...
For each: Ticker, Score, Elite, Entry, Stop, Target, Setup, Catalyst, Invalidation. **Include Leveraged alt** when applicable (e.g. NVDA→NVDU, AAPL→AAPU).
...
Context: 3 Stock Rotation (Trader One $20K): 60% Top 3 leveraged ETFs + 15% GDX + 15% FCX + 10% cash. Momentum = weeks to months.
This momentum scan supports both: rotation deploy ticker(s) (Top 3 from 26 leveraged ETFs) and individual stock momentum plays.
...
# Dip / Pullback scans: focus on support, invalidation, bounce catalyst, earnings avoidance.
DIP_DIRECTIVE = r"""
═══════════════════════════════════════════════════════════════════════════════
DIP / PULLBACK — AI Analysis Directive
═══════════════════════════════════════════════════════════════════════════════

You are a professional stock analyst. This is a DIP SCAN — stocks that have pulled back
from highs (e.g. -2% to -5% intraday or recent). Focus on BOUNCE setups, not momentum continuation.

TARGET AUDIENCE: Experienced trader. Output must be direct, actionable, no fluff.

Context: Swing trades (3–5 day hold), cash account. Position size by conviction. Stops at invalidation.

STRATEGY: BUY THE DIP when setup is clean.
- Support level: Where is the floor? (SMA50, prior low, round number)
- Invalidation: Price level that kills the thesis — break below = exit
- Catalyst: News/upgrade that justifies bounce? Or technical oversold only?
- EARNINGS: AVOID stocks with earnings in 1–3 days. Binary risk.
- Relative strength: Down less than sector/SPY = resilient. Down more = weakness.

CRITICAL — PRIORITIZE:
- Stocks ABOVE SMA200 (trend intact) > below
- Analyst Buy/Strong Buy > Hold
- News POSITIVE or neutral > DANGER
- Invalidation_level clearly defined

REQUIRED OUTPUT FORMAT:

📊 MARKET SNAPSHOT
──────────────────
[Date] Indices. Is this a buy-the-dip day (broad weakness) or stock-specific?

🎯 TIER 1: CLEAN DIP SETUPS
═══════════════════════════════════════════════════
#. [TICKER] - [COMPANY]
   Score: [X] | Price: $[X] | Pullback: [X]% from [level]
   Support: $[X] | Invalidation: $[X] (break below = exit)
   Catalyst: [News/technical — why bounce?]
   Entry: $[X]-[X] | Stop: $[X] | Target: $[X]
   [Leveraged alt when applicable]

🎯 TIER 2 / TIER 3: [Same format — add ⚠️ if earnings soon, weak catalyst]

❌ AVOID — EARNINGS TOO CLOSE / WEAK SETUP / NEWS DANGER

⚠️ RISK MANAGEMENT
• Stops: At invalidation (support break)
• Hold: 3–5 days. Exit on bounce or invalidation.
• Earnings: Never enter within 2 days of earnings.

TOP 5 PLAYS: Cleanest dip setups with clear invalidation.
...
class ReportGenerator:
    ...
    def _news_tag(self, title: str, ticker: str, company: str, sector: str) -> str:
        """Tag news: direct_mention (ticker/company in title), sector (sector in title), or market."""
        t = (title or "").upper()
        ticker_upper = (ticker or "").strip().upper()
        company_upper = (company or "").strip().upper()
        sector_upper = (sector or "").strip().upper()
        if ticker_upper and len(ticker_upper) >= 2 and ticker_upper in t:
            return "direct_mention"
        if company_upper and len(company_upper) >= 4:
            # Check for significant company name (avoid "Inc", "Corp")
            for word in company_upper.replace(",", " ").split():
                if len(word) >= 4 and word in t:
                    return "direct_mention"
        if sector_upper and sector_upper not in ("N/A", "") and sector_upper in t:
            return "sector"
        return "market"
    ...
    def _build_analysis_package(self, stocks_data, scan_type, timestamp_display, watchlist_matches, config=None, instructions=None, market_breadth=None, market_intel=None, price_history=None):
        ...
        for s in stocks_data:
            ...
            for item in news_list[:10]:
                if isinstance(item, (list, tuple)) and len(item) >= 2:
                    h = {"url": str(item[0]) if item[0] else "", "title": str(item[1]).strip()}
                elif isinstance(item, dict):
                    h = {"url": str(item.get("url", "")), "title": str(item.get("title") or item.get("headline", "")).strip()}
                else:
                    h = {"url": "", "title": str(item).strip()}
                h["tag"] = self._news_tag(h.get("title", ""), ticker, company, sector)
                headlines.append(h)
            row = {
                "ticker": s.get("ticker", ""),
                "score": s.get("score", 0),
                ...
                "wait_for_reclaim": _is_below_ema8(s.get("ema8_status"), s.get("ta")),
        ...
        # Rotation signal: sector or single-stock based on ptm_mode
        try:
            cfg = config or {}
            if cfg.get("ptm_mode") == "single_stock_rotation":
                from single_stock_rotation import get_single_stock_rotation_signal_for_report
                out["rotation_signal"] = get_single_stock_rotation_signal_for_report(lookback_days=5, config=config)
            else:
    ...
    def generate_combined_report_pdf(self, results, scan_type="Scan", min_score=60, progress_callback=None, watchlist_tickers=None, config=None, index=None):
        ...
        for s in stocks_data:
            ...
            if _is_below_ema8(s.get("ema8_status"), s.get("ta")):
                line += " | ⚠️ WAIT FOR RECLAIM (price below EMA8, entry not triggered)"
        ...
        try:
            ...
            if cfg.get("ptm_mode") == "single_stock_rotation":
                from single_stock_rotation import get_single_stock_rotation_signal_for_report
                rot = get_single_stock_rotation_signal_for_report(lookback_days=5, config=config)
            else:
                from sector_rotation import get_rotation_signal_for_report
                rot = get_rotation_signal_for_report(lookback_days=5, config=config)
            if rot and rot.get("top_ticker"):
                if rot.get("mode") == "single_stock_rotation":
                    t3 = rot.get("top_3_tickers", [])
                    if t3:
                        alloc = " | ".join(f"{p[3]:.0f}% {p[0]} ({p[1]}, +{p[2]:.1f}%)" for p in t3)
                        rot_prompt = f"\n3 STOCK ROTATION ($20K): 60% Top 3 ({alloc}) + 15% GDX + 15% FCX + 10% cash. Top 5: {', '.join(f'{r[1]}(+{r[2]:.1f}%)' for r in rot.get('rankings', [])[:5])}. 5% stop per rotation position.\n"
                    else:
                        rot_prompt = f"\n3 STOCK ROTATION ($20K): Deploy 100% into {rot['top_ticker']} ({rot.get('top_sector')}, +{rot.get('top_return_5d', 0):.2f}% 5d). 5% stop.\n"
                elif rot.get("n_positions") == 2 and rot.get("top_2_tickers"):
            ...
            pass
        # Scan-type-specific directives
        st_lower = scan_type.lower()
        is_momentum = "velocity" in st_lower or "velocity_trend" in st_lower
        is_dip = "dip" in st_lower or "emotional" in st_lower or "enhanced_dip" in st_lower
        if is_momentum:
            directive_block = MOMENTUM_TREND_DIRECTIVE.strip()
        elif is_dip:
            directive_block = DIP_DIRECTIVE.strip()
        else:
            directive_block = MASTER_TRADING_REPORT_DIRECTIVE.strip()
...
At the end of your analysis, add one line: CONSENSUS_VOTE: TICKER:BUY|SKIP|AVOID for each stock you discussed. Example: CONSENSUS_VOTE: AAPL:BUY, NVDA:BUY, MSFT:SKIP, META:AVOID
"""
        ...
        if cfg.get("ptm_mode") == "single_stock_rotation":
            body_lines = [
                "",
                "═══════════════════════════════════════════════════",
                "3 STOCK ROTATION (60% Top 3 + 15% GDX + 15% FCX + 10% cash)",
                "═══════════════════════════════════════════════════",
                ""
            ]
            try:
        ...
        if rot and rot.get("top_ticker"):
            if rot.get("mode") == "single_stock_rotation":
                t3 = rot.get("top_3_tickers", [])
                if t3:
                    body_lines.append("**3 Stock Rotation** — 60% Top 3 + 15% GDX + 15% FCX + 10% cash")
                    body_lines.append("**Rotation slice (60%):**")
                    for ticker, name, ret, weight in t3:
                        body_lines.append(f"  • {weight:.0f}% **{ticker}** ({name}, +{ret:.2f}% 5d)")
                    body_lines.append("**Hold:** GDX 15%, FCX 15% | **Cash:** 10%")
                else:
                    body_lines.append(f"**Deploy 100% into: {rot['top_ticker']}** ({rot.get('top_sector', 'N/A')}, +{rot.get('top_return_5d', 0):.2f}% 5d)")
                body_lines.append("")
                body_lines.append("Top 5 (5d momentum):")
                for r in rot.get("rankings", [])[:5]:
                    body_lines.append(f"  • {r[1]} ({r[0]}): +{r[2]:.2f}%")
                body_lines.append("")
                body_lines.append("*Strategy: 3 Stock Rotation — Weekly Top 3 from 26 leveraged ETFs (60%) + GDX/FCX (30%) + cash (10%). 5% stop per rotation position.*")
            elif rot.get("n_positions") == 2 and rot.get("top_2_tickers"):
                t1, t2 = rot["top_2_tickers"][:2]
